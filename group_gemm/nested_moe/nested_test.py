...
from expert_permute_nested import expert_input_permute, expert_output_permute
...
class ExpertsGroupGEMM(nn.Module):
    ...
    def forward(self, group_token):     
        group_token.tokens = group_token.tokens @ self.weights1
        group_token.tokens = self.act(group_token.tokens)
        group_token.tokens = group_token.tokens @ self.weights2       
        return group_token
...
class MoEGroupGEMM(nn.Module):
    ...
    def forward(self, tokens):
        ...
        output = expert_output_permute(grouped_tokens, expert_scores, tokens.shape)
        return output
