...
#include <cuda_fp16.h>
// TODO: tell vscode about cutlass headers...
...
void cutlass_group_gemm(
  ...
  thrust::device_vector<int64_t> gmm_args = host_gmm_args;//.to(device, true);
...
int main() {
  ...
  cutlass_group_gemm<
    ...
    /*WarpShape*/ cutlass::gemm::GemmShape<32, 32, 32>,
  ...
  std::vector<thrust::host_vector<scalar_t>> output(problem_count);
  for (int i = 0; i < problem_count; ++i) {
    output.at(i) = c.at(i);
  }

  std::cout << "should be 128:" << '\n';
  std::cout << output.at(0)[0] << '\n';
