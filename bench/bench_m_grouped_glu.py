...
def equal_groups_sizes(num_tokens, num_groups):
    return torch.arange(0, num_tokens, num_tokens // num_groups, device="cuda")
    
def m_grouped_glu_benchmark_extensive(
    plot_name, 
    routing_func,
    num_groups, 
    N, 
    ...
    line_vals = ["gemm-reference", "grouped-only", "grouped-gather"]
    line_names = ["GEMM-reference", "Grouped-only", "Grouped+Gather"]
    return Benchmark(
        x_names=["num_tokens"],
        x_vals=list(range(32, 8192 + 1, 128)),
        line_arg="provider",
        line_vals=line_vals,
...
configs = []
configs.append(m_grouped_glu_benchmark_extensive(
...
def benchmark_m_grouped_glu_only(num_tokens, num_groups, N, K, topk, dtype, p):
    num_tokens = num_tokens * topk
    input = torch.randn((num_tokens, K), device=torch.device("cuda"), dtype=dtype)
    weight = torch.randn((num_groups, K, N * 2), device=torch.device("cuda"), dtype=dtype) / math.sqrt(N)
    ...
    return bench(lambda: m_grouped_glu_interleaved(input, params, AutotuneMode.NONE))

def benchmark_m_grouped_glu_gather(num_tokens, num_groups, N, K, topk, dtype, p):
    input = torch.randn((num_tokens, K), device=torch.device("cuda"), dtype=dtype)
    weight = torch.randn((num_groups, K, N * 2), device=torch.device("cuda"), dtype=dtype) / math.sqrt(N)
    params = MGroupedGLUInterleavedParams(
        weight,
...
def benchmark_gemm_reference(num_tokens, num_groups, N, K, topk, dtype):
    ...
    Note: This does not include the GLU activation and product. This is just
    a matmul, of a similar size to the other benchmarks.
    """
    num_tokens = num_tokens * topk
...
def grouped_flops(num_tokens, num_groups, N, K, topk):
    num_tokens = num_tokens * topk
    return num_tokens * (2 * N) * K * 2

@perf_report(configs)
def benchmark_m_grouped_glu_forward(
    routing_func,
    num_tokens,
    num_groups,
    N,
    ...
    if provider in ("grouped-only", "grouped-gather"):
        ...
        if provider == "grouped-only":
            ms, _, _ = benchmark_m_grouped_glu_only(num_tokens, num_groups, N, K, topk, dtype, p)
        if provider == "grouped-gather":
            ms, _, _ = benchmark_m_grouped_glu_gather(num_tokens, num_groups, N, K, topk, dtype, p)
            
    flops = grouped_flops(num_tokens, num_groups, N, K, topk)
    tflops = flops / (ms / 1000) * 1e-12
    return tflops

benchmark_m_grouped_glu_forward.run(print_data=True, save_path="./")