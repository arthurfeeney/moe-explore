...
from moe_explore.testing import random_groups, random_routing, random_skewed_routing
...
configs.append(
        m_grouped_gemm_benchmark(
            ...
            routing_func=partial(random_skewed_routing, num_skewed_experts=64, skew_factor=4),
            num_groups=128,
            N=2048,
            K=768,
            topk=8,
...
configs.append(
        m_grouped_gemm_benchmark(
            ...
            routing_func=partial(random_skewed_routing, num_skewed_experts=32, skew_factor=4),
            num_groups=64,
            N=2048,
            K=1024,
            topk=8,
...
def benchmark_m_grouped_gemm_only(num_tokens, num_groups, N, K, topk, dtype):
    ...
    return do_bench(lambda: fused_moe(input, params, AutotuneMode.FAST), quantiles=quantiles)
...
def benchmark_gemm_reference(num_tokens, num_groups, N, K, topk, dtype):
    ...
    weight = torch.randn((num_groups, K, N), device=torch.device("cuda"), dtype=dtype) / math.sqrt(N)
    quantiles = [0.5, 0.2, 0.8]
    return do_bench(lambda: torch.compile(torch.matmul)(input, weight), quantiles=quantiles)
...
torch.manual_seed(0)
benchmark_m_grouped_gemm_forward.run(print_data=True)